...
Purpose: This script will check Linux hosts for ICMP and SSH connectivity.  The hostnames
        ...
        opened on default SSH port 22 to test connectivity.  The result of the host check
        will be displayed and color coordinated to indicate status.

        Output format:
        Host: hostname
        ICMP (Green = Responding to ICMP, Red = Not Responding to ICMP)
        SSH (Green = Accessible via SSH, Red = Not accessible via SSH)
...
Function ScriptMode($Run_Mode) {
    ...
    If ($Run_Mode -like "A"){         
        ...
        # Call the function to check the collected hostnames for ICMP and SSH connectivity
        CheckLinuxHosts $HostNames
...
Function CheckLinuxHosts ($HostNames){
    ...
    # Create a new TCP object to test for SSH connectivity
    Try {
            # Create a new socket and attempt to connect to the host via SSH default port
            $SSH_Socket = New-Object System.Net.Sockets.TCPClient($Host_Name,22)

		    If ($SSH_Socket -eq $null){

            # This indicates that the SSH connection failed, and will be caught by the catch statement

            } Else {
                
                # RDP connection is sucessful set output color to green
                Write-Host -ForegroundColor Green "SSH`n"
    ...
    Catch {
        ...
        # SSH connection to the host failed, output the SSH status as Red
        Write-Host -ForegroundColor Red  "SSH`n"
